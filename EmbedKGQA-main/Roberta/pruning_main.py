...
def process_data_file(fname, rel2idx, idx2rel):
    ...
    return data  #TODO  data格式为元组【问题，关系ID（可能有多个），问题加主实体】
...
def train(batch_size, shuffle, num_workers, nb_epochs, gpu, use_cuda, patience, validate_every, lr, decay, ls):
    ...
    # TODO 下面这个函数主要获取三个东西，问题词表数据question_token  ，attention_mask和关系的one-hot
    ...
    for epoch in range(nb_epochs):
        ...
        for i in range(validate_every):
            phases.append('train')
        phases.append('valid')
        for phase in phases:#todo 每25次训练进行一次验证
            if phase == 'train':
                model.train()
